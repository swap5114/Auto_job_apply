...
def deploy_to_github(
    ...
    # Retrying a deploy for the same build_id reuses this exact repo_name
    # (it's deterministic: prefix + company slug + build_id). If a PREVIOUS
    # attempt already created and pushed it — e.g. GitHub succeeded but a
    # later Vercel/Render step failed, and the user retried the whole
    # deploy — `gh repo create` on an existing name fails outright. Check
    # first and push to the existing remote instead of trying to recreate it.
    existing_info = _get_existing_repo(repo_name)

    if existing_info:
        print(f"  Repo {repo_name} already exists — pushing to it instead of recreating.")
        repo_url = existing_info["url"]
        owner = existing_info["nameWithOwner"].split("/")[0]
        _run(["git", "remote", "add", "origin", f"https://github.com/{existing_info['nameWithOwner']}.git"], cwd=project_dir)
        # force-with-lease would be safer in a general-purpose tool, but this
        # repo is fully owned/generated by this pipeline (never hand-edited),
        # so a plain force-push reflects "this retry's build output is the
        # new source of truth" without any real risk of clobbering someone
        # else's work.
        _run(["git", "push", "-f", "origin", "main"], cwd=project_dir, timeout=GIT_COMMAND_TIMEOUT)
    else:
        create_result = _run(
...
def _get_existing_repo(repo_name: str) -> Optional[dict]:
    """Return {"url": ..., "nameWithOwner": ...} if a repo with this name
    already exists under the authenticated gh account, else None. Does not
    raise on a "not found" result — that's the expected/common case.
    """
    result = subprocess.run(
        ["gh", "repo", "view", repo_name, "--json", "url,nameWithOwner"],
