...
import { CheckCircle2, Edit3, XCircle, ChevronDown, Loader2, RefreshCw } from "lucide-react";
...
export default function ReviewPage() {
  const [allPending, setAllPending] = useState<Lead[]>([]);
  ...
  async function load() {
    ...
    setError(null);
    try {
      // Leads awaiting review: pending_review + in_review
      ...
      setAllPending(combined);
      setLeads(combined);
      setExpandedId(combined[0]?.id ?? null);
    } catch (e: any) {
          ...
          {leads.length === 0 && (
            ...
            >
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                No leads pending review right now. Run the pipeline to queue more.
              </p>
            </motion.div>
          )}
